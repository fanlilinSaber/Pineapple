...
- (void)drawInContext:(CGContextRef)context withPosition:(CGPoint)p andFont:(UIFont *)font andTextColor:(UIColor *)color andHeight:(float)height andWidth:(float)width andLenghtHeight:(float)LenghtHeight{
    ...
    /*&* 如果为0  则默认显示为一行*/
    if (LenghtHeight == 0) {
        size = CGSizeMake(width, font.pointSize + 10);
    }
    else
    {
        size = CGSizeMake(width, LenghtHeight + 20);
    }
    ...
    //Apply paragraph settings
    CGFloat minimumLineHeight = font.pointSize,maximumLineHeight = minimumLineHeight+10, linespace = 4;
    CTLineBreakMode lineBreakMode = kCTLineBreakByWordWrapping;
    CTTextAlignment alignment = kCTLeftTextAlignment;
...
- (void)drawInContext:(CGContextRef)context withPosition:(CGPoint)p andFont:(UIFont *)font andTextColor:(UIColor *)color andHeight:(float)height{
    [self drawInContext:context withPosition:p andFont:font andTextColor:color andHeight:height andWidth:CGFLOAT_MAX andLenghtHeight:0];
}
